...
func ServiceHealthCheck(c *gin.Context) {
	...
	go func() {
		userServiceStatus := checkServiceHealth("http://user_service:8800/health", 5*time.Second)
	...
	go func() {
		serverServiceStatus := checkServiceHealth("http://server_service:8801/health", 5*time.Second)
	...
	go func() {
		reportServiceStatus := checkServiceHealth("http://report_service:8802/health", 5*time.Second)
